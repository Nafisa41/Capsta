...
def step_update():
  ...
  for file_name in file_name_list:
    obj_cor = deserialize_temp("adult male night_26.pk")
    graph_assc_cor = obj_cor.graph
    ...
    for keyvalue in node_list:
      try:
        lst_tempD = graph_assc_cor.nodes[keyvalue]['tempD']
        sum_tempD = sum(lst_tempD)
        if(graph_assc.nodes[keyvalue]['prev_mean'] == 0.0 and len(lst_tempD) == 0.0):
          continue
        favg = (graph_assc.nodes[keyvalue]['prev_mean'] * 0.5 * graph_assc.nodes[keyvalue]['crime_count'] + sum_tempD) / ((0.5 * graph_assc.nodes[keyvalue]['crime_count']) + len(lst_tempD))
        ...
        graph_assc.nodes[keyvalue]['crime_count'] = (0.5 * graph_assc.nodes[keyvalue]['crime_count']) + len(lst_tempD)
        #flush
    ...
    for u, v, k in edge_list:
      try:
        lst_tempD = graph_assc_cor[u][v][k]['tempD']
        sum_tempD = sum(lst_tempD)
        if(graph_assc.nodes[keyvalue]['prev_mean'] == 0.0 and len(lst_tempD) == 0.0):
          continue
        favg = (graph_assc[u][v][k]['prev_mean'] * 0.5* graph_assc[u][v][k]['crime_count'] + sum_tempD) / ((0.5 * graph_assc[u][v][k]['crime_count']) + len(lst_tempD))
        risk_value = abs(favg - obj.global_best) / (abs(favg - obj.global_worst) + abs(favg - obj.global_best))
        graph_assc[u][v][k]['risk'] = risk_value
        graph_assc[u][v][k]['prev_mean'] = favg
        graph_assc[u][v][k]['crime_count'] = (0.5 * graph_assc[u][v][k]['crime_count']) + len(lst_tempD)
        #flush
